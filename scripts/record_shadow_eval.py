...
def _latest_shadow_gate_decision(telemetry_dir: Path, proposal_id: str) -> dict[str, Any] | None:
    latest: dict[str, Any] | None = None
    latest_key: tuple[int, int] | None = None
    line_index = 0
    gate_decisions = {"ACCEPT_SHADOW", "REJECT", "ACCEPT_CANARY", "ACCEPT_ROLLOUT", "ROLLBACK"}
...
def _shadow_eval_link_count(telemetry_dir: Path, proposal_id: str) -> int:
    count = 0
    for path in sorted(telemetry_dir.glob("proposal_links-*.jsonl")):
        with path.open("r", encoding="utf-8") as handle:
            for line in handle:
                ...
                if str(row.get("proposal_id") or "") != proposal_id:
                    continue
                if str(row.get("link_type") or "") != "shadow_eval":
                    continue
                count += 1
    return count


def _latest_shadow_eval_report_id(telemetry_dir: Path, proposal_id: str) -> str | None:
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Append a shadow eval report and proposal link.")
    ...
    ap.add_argument("--method", default="replay_eval")
    ...
    ap.add_argument("--force", action="store_true", help="Allow duplicate shadow eval for same proposal_id.")
    ...
    latest = _latest_shadow_gate_decision(telemetry_dir, str(args.proposal_id))
    decision = str((latest or {}).get("decision") or "")
    actor = str((latest or {}).get("actor") or "")
    if decision != "ACCEPT_SHADOW" or actor not in {"human", "auto"}:
        print(
            f"[error] proposal_id={args.proposal_id} is not approved for shadow eval (latest decision={decision or 'none'})",
            file=sys.stderr,
        )
        raise SystemExit(2)
    previous_eval_report_id = _latest_shadow_eval_report_id(telemetry_dir, str(args.proposal_id))
    if not args.force:
        existing_links = _shadow_eval_link_count(telemetry_dir, str(args.proposal_id))
        if existing_links >= 1:
            print(
                f"[error] shadow eval already exists for proposal_id={args.proposal_id}; use --force to override",
                file=sys.stderr,
            )
            raise SystemExit(4)
